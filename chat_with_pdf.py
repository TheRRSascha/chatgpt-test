...
chunk_sentence (Deprecated): This collection was generated by an older version of 'by_sentence.py'. It used a method that 
created chunks on a sentence-by-sentence basis, aiming for a maximum length. However, some chunks exceeded this limit.

chunk_sentence-experimental: This collection is the result of an updated approach from the current 'by_sentence.py'. It 
enhances chunking while strictly maintaining sentence length. Any chunks that exceed the maximum length are dropped, 
with their details recorded in a log file. 
